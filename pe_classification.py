...
from models import PatchEmbed, LinearClsHead, AttnBlocks
...
def main(args):
    ...
    if dataset_name == 'NWPU_RESISC45':
        ...
        # build model
        tokenizer = PatchEmbed(patch_size=16, with_cls_token=True)
        backbone = AttnBlocks(depth=args.backbone_depth)
        taskHead = LinearClsHead(num_classes=45, in_channels=768, mode="cls_token")
        model = nn.Sequential(tokenizer, backbone, taskHead)
        model.cuda()
        ...
        tokenizer = PatchEmbed(patch_size=16, with_cls_token=True)
        backbone = AttnBlocks(depth=args.backbone_depth)
        taskHead = LinearClsHead(num_classes=19, in_channels=768, mode="cls_token")
        model = nn.Sequential(tokenizer, backbone, taskHead)
        model.cuda()
    ...
    for cur_epoch in range(max_epoch):
        ...
        if (cur_epoch+1) % args.eval_epochs == 0 or cur_epoch == max_epoch-1:
            evaluation(model, test_loader, test_dataset, args, myLogger=myLogger)
            
    print("Done!")
    
@torch.no_grad()
def evaluation(model, test_loader, test_dataset, args, myLogger):
    model.eval()
